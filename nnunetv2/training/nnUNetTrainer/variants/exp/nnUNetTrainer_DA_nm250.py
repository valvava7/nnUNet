...
class nnUNetTrainer_DA3_nm250(nnUNetTrainer_250epochs_NoMirroring):
    ...
    def get_training_transforms(
        ...
        transforms.append(RandomTransform(
            GaussianBlurTransform(
                blur_sigma=(0.3, 0.6), #(0.5,1.),
                synchronize_channels=False,
                synchronize_axes=False,
                p_per_channel=0.5, benchmark=True
            ), apply_probability=0.1 # 0.2
        ))
        transforms.append(RandomTransform(
            MultiplicativeBrightnessTransform(
                multiplier_range=BGContrast((0.9, 1.1)), # (0.75, 1.25)
                synchronize_channels=False,
                p_per_channel=1
            ), apply_probability=0.1 # 0.15
        ))
        transforms.append(RandomTransform(
            ContrastTransform(
                contrast_range=BGContrast((0.9, 1.1)), # (0.75, 1.25)
                preserve_range=True,
                synchronize_channels=False,
                p_per_channel=1
            ), apply_probability=0.1 # 0.15
        ...
        transforms.append(RandomTransform(
            GammaTransform(
                gamma=BGContrast((0.85, 1.25)), #(0.7,1.5)
                p_invert_image=0,
                synchronize_channels=False,
                p_per_channel=1,
                p_retain_stats=1
            ), apply_probability=0.2 #0.3
        ))
        if mirror_axes is not None and len(mirror_axes) > 0:
            transforms.append(
                MirrorTransform(
...
#add elastic deform
class nnUNetTrainer_DA5_nm250(nnUNetTrainer_250epochs_NoMirroring):
    @staticmethod
    def get_training_transforms(
            patch_size: Union[np.ndarray, Tuple[int]],
            rotation_for_DA: RandomScalar,
        ...
        transforms.append(
            SpatialTransform(
                patch_size_spatial, patch_center_dist_from_border=0, random_crop=False, p_elastic_deform=0.1, # here 0.0
                p_rotation=0.2,
                rotation=rotation_for_DA, p_scaling=0.2, scaling=(0.7, 1.4), p_synchronize_scaling_across_axes=1,
                bg_style_seg_sampling=False  # , mode_seg='nearest'
            )
...
#add local gamma
class nnUNetTrainer_DA6_nm250(nnUNetTrainer_250epochs_NoMirroring):
    @staticmethod
    def get_training_transforms(
            patch_size: Union[np.ndarray, Tuple[int]],
            rotation_for_DA: RandomScalar,
